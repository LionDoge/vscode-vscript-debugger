...
import { activateVScriptDebug, workspaceFileAccessor } from './activateVScriptDebug';
...
export function activate(context: vscode.ExtensionContext) {
	...
	switch (runMode) {
		case 'server':
			...
			activateVScriptDebug(context, new VScriptDebugAdapterServerDescriptorFactory());
			break;

		case 'namedPipeServer':
			// run the debug adapter as a server inside the extension and communicate via a named pipe (Windows) or UNIX domain socket (non-Windows)
			activateVScriptDebug(context, new VScriptDebugAdapterNamedPipeServerDescriptorFactory());
			break;

		case 'external': default:
			// run the debug adapter as a separate process
			activateVScriptDebug(context, new DebugAdapterExecutableFactory());
			break;

		case 'inline':
			// run the debug adapter inside the extension and directly talk to it
			activateVScriptDebug(context);
			break;
	}
}
...
class VScriptDebugAdapterServerDescriptorFactory implements vscode.DebugAdapterDescriptorFactory {
	...
	createDebugAdapterDescriptor(session: vscode.DebugSession, executable: vscode.DebugAdapterExecutable | undefined): vscode.ProviderResult<vscode.DebugAdapterDescriptor> {
		...
		if (!this.server) {
			...
			this.server = Net.createServer(socket => {
				const session = new VScriptDebugSession(workspaceFileAccessor);
...
class VScriptDebugAdapterNamedPipeServerDescriptorFactory implements vscode.DebugAdapterDescriptorFactory {
